        ...
        BaseMain { insets, deviceType in
            ...
            VStack(spacing: 0) {
                HStack(spacing: 15) {
                    Spacer()
                    Button {
                        Navigator.shared.push(to: AccountsScreen(title: "Notification"))
                    } label: {
                        ZStack {
                ...
                ScrollView(.vertical, showsIndicators: false) {
                    VStack(spacing: 0) {
                        ...
                        UserAccountBannerComponent(
                            payAndTransferAction: {
                                Navigator.shared.push(to: AccountsScreen(title: "Pay & Transfer"))
                            }
                        )
                        ...
                        BankingServiceComponent(
                            ...
                            bgColor: Color.black,
                            items: [
                                ItemModel(icon: "ic_accounts", title: "Accounts", action: { Navigator.shared.push(to: AccountsScreen(title: "Accounts")) }),
                            ...
                            extraItems: [
                                ...
                                ItemModel(icon: "ic_locator", title: "Locator", action: { Navigator.shared.push(to: AccountsScreen(title: "Locator")) }),
                            ]
                        )
                        .padding(.vertical, 12)
                        ...
                        FrequentlyUsedComponent(
                            items: [
                                ...
                                ItemModel(icon: "ic_loan", title: "Loan"),
                                ItemModel(icon: "ic_cards", title: "Cards"),
                                ItemModel(icon: "ic_favorites", title: "Favorites", action: { Navigator.shared.push(to: AccountsScreen(title: "Favorites")) }),
                            ]
                        )
